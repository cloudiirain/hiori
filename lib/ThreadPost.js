...
class ThreadPost {
   ...
   * @property {Cheerio} message The cheerio selector for the post message
  ...
  constructor (li) {
    try {
      ...
      this.timestamp = li.find('.datePermalink .DateTime').first();
      ...
      const isRecent = this.timestamp.attr('data-time');
      if (isRecent) {
        // Recent posts have a timestamp associated with them
        this.timestamp = parseInt(isRecent);
      } else {
        // Later posts only have the string date associated with them
        const textDate = this.timestamp.text().trim();
        this.timestamp = new Date(textDate).getTime();
      }

    } catch (e) {
      throw new ParserError('Error parsing NUF thread page post');
    }
  }
